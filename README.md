...
| Step | What happens |
|---|---|
| Request in Backstage | A pull request adds `resources/<type>/<environment>/<name>/` (Terraform using a vetted module + `catalog-info.yaml`) |
...
| Connect to a service | Optional second PR in the service repo adds `infra/app/bindings/<env>/<type>-<name>.json` (access + environment variables) |
